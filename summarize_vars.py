...
#TODO
#modify queries to incorporate non-census geography for summarization
#if geography is not census then non-census geography needs to be appended
    #to from parameters in selection query
    """
    select {summary_geography}, [fields]
    from
        (subquery)<,non_census_geography
    spatial_join>
    group by {summary_geography}
    """
...
'38120', '38115','38053','38138','38114','38108','38122','38018','38132','38126',
    '38119','38054','38139','38118','38135','38103','38141','38028','38017',
    '38133','38134','38111','38131','38036','38152','38117','38016','38128',
    '38116','38029','38105']
...
def tot_pop(geography):
    ...
    q = """alter table {0} add column {2} numeric(20,2); update {0} \
        set {2} = subquery.{2} \
        from (select c.geoid, b01003001 as {2} \
        from {1}.b01003 as c) as subquery \
        where {0}.geoid10 = subquery.geoid;"""
...
def hhinc(geography):
    ...
    add_cols = "add column {column} numeric(20,2), "
...
def pct_race(geography):
    ...
    q = """alter table {0} add column pct_wh numeric(20,2), \
            ...
            from {1}.b02001 as t, {0}) as subquery \
            where {0}.geoid10 = subquery.geoid
        """
...
#     q = """alter table {0} add column {1}; update {0} \
#     set {2} \
...
def pct_poverty(geography):
    ...
    fields = ['pct_pov_to14','pct_pov_15to17',
                'pct_pov_18to24','pct_pov_25to34',
		'pct_pov_35to44','pct_pov_45to54',
		'pct_pov_55to64','pct_pov_65up']
    add_columns = ', '.join("add column {} numeric(20,2)".format(
    ...
    q = """alter table {0} \
            ...
            from {1}.b17001 as t, {0}) as subquery \
            where {0}.geoid10 = subquery.geoid
        """
    cursor.execute(q.format(geography, t_schema))
...
def pct_poverty_race(geography):
    ...
    q = """alter table {0} add column {1} numeric(20,2); update {0} \
    set {1} = subquery.{1} \
...
def pct_by_sex(geography):
    ...
    q = """alter table {0} add column {2} numeric(20,2), \
            ...
            from {1}.b01001 as t, {0}) as subquery \
            where {0}.geoid10 = subquery.geoid
        """
...
def housing_density(geography):
    ...
    col = 'hsng_density'
    q = """alter table {0} drop column if exists {2},
    add column {2} numeric(20,2); update {0} \
    set {2} = subquery.{2}/{0}.sqmiland \
    from (select c.geoid, b25001001 as {2} \
    from {1}.b25001 as c, {0}) as subquery \
    where {0}.geoid10 = subquery.geoid;"""
    cursor.execute(q.format(geography,t_schema, col))
...
def sfcomm(geography):
    ...
    alter table {geography} drop column if exists {col}, \
    ...
    where {geography}.geoid10 = subquery.geoid10 \
    and {query};\
    """
#     q = """alter table {0} add column sfcommacre numeric(20,2); \
#     update {0} \
...
#     ) as subquery \
#     where {0}.geoid10 = subquery.geoid10;"""
...
def age_comm(geography):
    ...
    q = """alter table {geography} add column {col} numeric(20,2);
    update {geography} \
    set {col} = subquery.{col} \
    from \
...
def pct_vacant(geography):
    ...
    params = {'geography': geography,
              ...
              'query': geog_key[geography],
              'parcels':parcels}
    q = """alter table {geography} drop column if exists {col},
    add column {col} numeric(20,2); \
    update {geography} \
    set {col} = subquery.{col} \
    from \
...
def afford_hsng_green(geography):
    ...
    col = 'affhsgreen'
    q = """alter table {0} add column {3} numeric(20,2); \
    update {0} \
    set {3} = subquery.{3} \
    from \
    (select t.geoid10, \
...
def pct_own_race(geography):
    poverty_race_tables = {'a': 'wh', 'b': 'bl', 'c': 'ai',
                           ...
                           'g':'2m', 'i': 'hp'}
    t_schema = schema['b25003']
    q = """alter table {0} add column {1} numeric(20,2); \
        update {0} set {1} = subquery.{1} \
        from \
        (select c.geoid, \
        case b.b25003001 \
        when 0 then 0 \
...
def pct_mf(geography):
    ...
    q = """alter table {0} add column {3} numeric(20,2); update {0} \
    set {3} = subquery.{3} \
    from (select c.geoid, \
    case b25032001 \
    when 0 then 0 \
...
def pct_streets_sdw(geography):
    ...
    q = """alter table {0} add column strtsdw_pct numeric(20,2); \
            update {0} \
...
def bikeability(geography):
    ...
    q = """alter table {0} add column bic_index numeric(20,2);
    update {0} \
...
def mmode_connection(geography):
    ...
    q = """alter table {0} add column mmcnxpsmi numeric(20,2);
    update {0} \
...
def transit_access(geography):
    ...
    q = """alter table {0} add column transit_access numeric(20,2);
    update {0} \
...
def mata_stops(geography):
    ...
    q = """alter table {0} drop column if exists mata_stop_sqmi,
...
def bus_rider(geography):
    t_schema = schema['mata_stops']
    q = """alter table {0} add column rider_sqmi numeric(20,2);
    update {0} \
...
def vmt(geography):
    t_schema = schema['htaindex']
    q = """alter table {0} add column vmt_per_hh_ami numeric(20,2); update {0} \
...
def walkability(geography):
    ...
    q = """alter table {0} add column walkscore numeric(20,2);
    update {0} \
...
def car_own(geography):
    t_schema = schema['htaindex']
    q = """alter table {0} add column autos_per_hh_ami numeric(20,2); update {0} \
...
def tree_canopy(geography):
    ...
    if geography == 'cen_place_2010':
        q = """alter table {0} add column pct_canopy numeric(20,2);
        update {0} \
...
def elec_station(geography):
    ...
    q = """alter table {0} add column elec_chg_station numeric(20,2); \
    update {0} \
...
def green_bldgs(geography):
    ...
    q = """alter table {0} add column green_bldgs_sqmi numeric(20,2); \
    update {0} \
...
def air_qual(geography):
    ...
    q = """alter table {0} drop column if exists days_ovr_aq_stndrd,\
...
def chg_prop_val(geography):
    ...
    if geography == 'cen_zip_2010':
        apr14 = """alter table {geography} drop column if exists rtotapr14,
        ...
        apr04 = """alter table {geography} drop column if exists rtotapr04,
        ...
        group by t.geoid10) as subquery \
        where {geography}.geoid10 = subquery.geoid10 \
        and substring({geography}.geoid10,1,5) = '47157';
        """
    ...
    pct_change = """alter table {geography} drop column if exists {col},
    add column {col} numeric(20,2); \
    update {geography} \
    ...
    rem = """alter table {geography} drop column rtotapr04, drop column rtotapr14;"""
    cursor.execute(rem.format(**params))

def snap_receipt(geography):
    t_schema = schema['b19058']
    q = """alter table {0} add column pct_snap numeric(20,2); update {0} \
...
def avg_hours_worked(geography):
    ...
    q = """alter table {0} add column avg_hours numeric(20,2); update {0} \
...
def job_access(geography):
    t_schema = schema['htaindex']
    q = """alter table {0} add column emp_ovrll_ndx numeric(20,2); update {0} \
...
def pct_labor_force(geography):
    ...
    q = """alter table {0} add column pct_labor_force numeric(20,2); update {0} \
...
def emp_mix(geography):
    t_schema = schema['htaindex']
    q = """alter table {0} add column emp_ndx numeric(20,2); update {0} \
...
def pct_unemp(geography):
    t_schema = schema['b23025']
    q = """alter table {0} add column pct_unemp numeric(20,2); update {0} \
...
def emp_sector(geography):
    ...
    jobs = {'cns01': 'pct_ag', 'cns02': 'pct_mining', 'cns03': 'pct_util',
            ...
            'cns16': 'pct_health', 'cns17': 'pct_arts', 'cns18': 'pct_food',
            'cns19': 'pct_other', 'cns20': 'pct_pubadmin'}
    q = """alter table {0} drop column if exists {1},
...
def low_mod_jobs(geography):
    t_schema = schema['emp_centers']
    q = """alter table {0} add column pct_lowinc_job numeric(20,2);
    update {0} \
...
def ed_attain(geography):
    ...
    q = """alter table {0} add column pct_{1} numeric(20,2); update {0} \
...
def elem_schools(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column elem_pcap numeric(20,2); \
#     update {0} \
#     set elem_pcap =
#     case totpop \
#     when 0 then 0 \
...
#     q = """alter table {0} add column middle_pcap numeric(20,2); \
#     update {0} \
#     set middle_pcap =
#     case totpop \
#     when 0 then 0 \
...
#     q = """alter table {0} add column high_pcap numeric(20,2); \
#     update {0} \
#     set high_pcap =
#     case totpop \
#     when 0 then 0 \
...
#     q = """alter table {0} add column pvt_pcap numeric(20,2); \
#     update {0} \
#     set pvt_pcap =
#     case totpop \
#     when 0 then 0 \
...
def daycare_dist(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column chldcntr_pcap numeric(20,2); \
#     update {0} \
#     set chldcntr_pcap = \
#     case totpop \
#     when 0 then 0 \
...
#     q = """alter table {0} add column cmgrdn_pcap numeric(20,2); \
#     update {0} \
#     set cmgrdn_pcap =
#     case totpop \
#     when 0 then 0 \
...
def farmers_markets(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column frmrmkt_pcap numeric(20,2); \
#     update {0} \
#     set frmrmkt_pcap =
#     case totpop \
#     when 0 then 0 \
...
#     q = """alter table {0} add column library_pcap numeric(20,2); \
#     update {0} \
#     set library_pcap =
#     case totpop \
#     when 0 then 0 \
...
def commcenters_dist(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#    alter table {0} add column commcenter_pcap numeric(20,2); \
#     update {0} \
#     set commcenter_pcap = \
#     case totpop \
#     when 0 then 0 \
...
def medicaid_pop(geography):
    ...
    q = """alter table {0} add column pct_medicaid numeric(20,2); update {0} \
...
def hospitals(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column hosp_pcap numeric(20,2); \
#     update {0} \
#     set hosp_pcap = \
#     case totpop \
#     when 0 then 0 \
...
def police_stations(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column polstn_pcap numeric(20,2); \
#     update {0} \
#     set polstn_pcap = \
#     case totpop \
#     when 0 then 0 \
...
def fire_stations(geography):
    ...
    if geography in ['cen_tract_2010', 'cen_zip_2010']:
        ...
        where {0}.geoid10 = subquery.geoid10
        """
#     q = """alter table {0} add column firestn_pcap numeric(20,2); \
#     update {0} \
#     set firestn_pcap = \
#     case totpop \
#     when 0 then 0 \
...
def os_sqmi(geography):
    ...
    q = """alter table {0} add column os_sqmi numeric(20,2);
    update {0} \
...
def wetlands_sqmi(geography):
    ...
    q = """alter table {0} add column wetland_sqmi numeric(20,2);
    update {0} \
...
def brownfld_sqmi(geography):
    ...
    q = """alter table {0} add column brnfld_sqmi numeric(20,2);
    update {0} \
...
def hisp_pop(geography):
    ...
    q = """alter table {0} add column pct_hisp numeric(20,2);
            update {0} \
                set pct_hisp =
                case when subquery.b03003003 = 0 then 0
                else subquery.b03003003/{0}.totpop *100\
                end \
...
def broadband(geography):
    ...
    q = """alter table {0} add column pcat_10 numeric(20);\
            update {0} \
            set pcat_10 = \
...
def life_exp(geography):
    ...
    if geography == 'cen_zip_2010':
        params = {'geography': geography, 't_schema':t_schema, 
                'column': 'life_exp'} 
        q = """alter table {geography} add column {column} integer;
                update {geography} set {column} = subquery.{column} \
                from (select zipcode, {column} from {t_schema}.schd_life_exp_0913) subquery \
                    where {geography}.geoid10 = subquery.zipcode;"""
        cursor.execute(q.format(**params))
    ...
    if geography == 'cen_tract_2010':
        ...
        for col in ['births11_15', 'deaths10_14', 'neoplasms', 'heartdis']:
            params = {'geography': geography, 'column': col, 
                      'schema': schema}
            q = """alter table {geography} add column {column} numeric(20,2);
                update {geography} set {column} = tdh_indicators.{column}\
